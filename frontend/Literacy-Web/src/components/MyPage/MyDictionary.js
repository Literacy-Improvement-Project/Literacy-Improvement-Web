...
export default function MyDictionary({ dictionaryWordsList, handleDelete }) {
    ...
    const myWordList = wordName.map((word, index) => (
        ...
        <Grid item key={index} xs={4} className={classes.paper}>
            ...
            <button className ={classes.deletebutton} onClick={() => wordDelete(wordName[index])}>삭제</button>
        </Grid>
    ))
    // 사용자에게 보여지는 부분
    return (
        <div className={classes.root}>
            ...
            alignItems="center">
                <Grid item xs={12}>
                    <Typography variant="h6" color="textPrimary">내 단어장</Typography>
                </Grid>
                {myWordList}
            </Grid>
        </div>
